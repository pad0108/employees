	...
	<body>
		...
		<div class="container center">
			<%
				...
				/*
					select dept_no, emp_no, from_date, to_date from dept_manager	-> dept_manager 테이블 참조
				...
				*/
		        ...
		        // SQL문
		        String sql = "select dept_no, emp_no, from_date, to_date from dept_manager order by emp_no desc limit ?, ?";
			...
			<ul class="pagination justify-content-center">
				...
				<%
					if (currentPage > 1) { // currentPage가 1보다 클 때만 처음으로 갈 수 있음
						%>
							<li class="page-item">
								<a class="page-link" href="./deptManagerList.jsp?currentPage=1">
				...
				<%
					if (currentPage > 1) { // currentPage가 1보다 클 때만 이전으로 갈 수 있음
						%>
							<li class="page-item">
								<a class="page-link" href="./deptManagerList.jsp?currentPage=<%=currentPage - 1 %>">
					...
					for(int i = navFirstPage; i <= navLastPage; i++) {
						if (i <= lastPage) {
							if (i == currentPage) {
								...
								%><li class="page-item"><a class="page-link" href="./deptManagerList.jsp?currentPage=<%=i %>"><%=i %></a></li><%	// 다른 페이지
							}
						}
					}
				%>
				...
				<%
					if (currentPage < lastPage) { // currentPage가 lastPage보다 작을 때만 마지막으로 갈 수 있음
						%>
							<li class="page-item">
								<a class="page-link" href="./deptManagerList.jsp?currentPage=<%=lastPage %>">
